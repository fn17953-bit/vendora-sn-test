  ...
  function render(){
    ...
    container.innerHTML=photos.map((ph,i)=>`
      <div>
        ...
        <div class="photo-slot${ph.url?' has-photo':''}" id="${containerId}_slot${i}" style="position:relative;overflow:hidden;${photoTabs[i]==='url'?'cursor:default;':''}">
          ${ph.url?`<img src="${ph.url}" alt="${ph.label}" style="pointer-events:none" onerror="this.style.display='none'">`:'' }
          ${photoTabs[i]==='upload'?`<input type="file" id="${containerId}_file${i}" accept="image/*,.heic,.heif" onchange="ptUpload_${containerId}(${i},this)" style="position:absolute;top:0;left:0;width:100%;height:100%;opacity:0;cursor:pointer;z-index:20;margin:0;padding:0;">`:'' }
          ...
          ${ph.url?`<button class="photo-remove" style="position:relative;z-index:30" onclick="ptClear_${containerId}(event,${i})">✕</button>`:''}
        </div>
        ${photoTabs[i]==='url'
          ?`<input type="text" class="photo-url-input" placeholder="https://... ou GIF" value="${ph.url||''}" oninput="ptSetUrl_${containerId}(${i},this.value)" onblur="ptPreview_${containerId}(${i},this.value)">`
          :`<p style="font-size:.6rem;color:var(--muted);text-align:center;margin-top:.3rem;font-family:var(--font-sub)">JPG · PNG · GIF · Max 5Mo</p>`}
      </div>`).join('');
